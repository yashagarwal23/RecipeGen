...
import time
from rnn_model.model import RNNModel
from rnn_model.attention_model import RNNAttentionModel
...
def evaluate(model, criterion, args, data_source, target_source, batch_size=10):
    ...
    for i in range(0, data_source.size(0) - 1 - 1, args["bptt"]):
        ...
        targets = get_batch(target_source, i+1, args, evaluation=True).view(-1)
...
def train(model, train_data, train_target, optimizer, criterion, params, epoch, args):
    ...
    while i < train_data.size(0) - 1 - 1:
        ...
        targets = get_batch(train_target, i+1, args, seq_len=seq_len).view(-1)
        ...
        if args["alpha"]: loss = loss + sum(args["alpha"] * dropped_rnn_h.pow(
            2).mean() for dropped_rnn_h in dropped_rnn_hs[-1:])
        # Temporal Activation Regularization (slowness)
        if args["beta"]: loss = loss + \
...
def train_and_eval(model, corpus, optimizer, criterion, params, args, save_path):
    val_data = batchify(corpus.valid_input, args["eval_batch_size"], args)[:-1]
    test_data = batchify(corpus.test_input, args["test_batch_size"], args)[:-1]
    train_data = batchify(corpus.train_input, args["batch_size"], args)[:-1]

    val_target = batchify(corpus.valid_target, args["eval_batch_size"], args)
    test_target = batchify(corpus.test_target, args["test_batch_size"], args)
    train_target = batchify(corpus.train_target, args["batch_size"], args)
    ...
    try:
        for epoch in range(1, args["epochs"] + 1):
            ...
            train(model, train_data, train_target, optimizer, criterion, params, epoch, args)
            if 't0' in optimizer.param_groups[0]:
                ...
                val_loss2 = evaluate(model, criterion, args, val_data, val_target)
                print('-' * 89)
                ...
                val_loss = evaluate(model, criterion, args,
                                    val_data, val_target, args["eval_batch_size"])
                print('-' * 89)
                print('| end of epoch {:3d} | time: {:5.2f}s | valid loss {:5.2f} | '
                      'valid ppl {:8.2f} | valid bpc {:8.3f}'.format(
    ...
    if os.path.exists(save_path):
        ...
        # Run on test data.
        test_loss = evaluate(model, criterion, args, test_data, test_target, args["test_batch_size"])
        print('=' * 89)
