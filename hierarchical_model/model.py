...
class hierarchical_model(nn.Module):
    def __init__(self, model_subtype_to_word, model_type_to_subtype, model_type):
        ...
        self.model_list = [model_subtype_to_word, model_type_to_subtype, model_type]
    ...
    def forward(self, input, hidden, return_h = False, emb_inp=False, emb_out=False):
        output_type, new_type_hidden = self.model_type(input, hidden[2], return_h=False, emb_out=True, emb_inp=emb_inp)

        output_type_to_subtype, new_type_to_subtype_hidden = self.model_type_to_subtype(input, output_type, hidden[1], return_h=False, emb_inp1=emb_inp, emb_inp2=True, emb_out=True)

        output, new_subtype_to_word_hidden, rnn_hs, dropped_rnn_hs = self.model_subtype_to_word(input, output_type_to_subtype, hidden[0], return_h=True, emb_inp1 = emb_inp, emb_inp2=True, emb_out=emb_out)
        if return_h:
